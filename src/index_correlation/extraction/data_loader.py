...
from index_correlation.extraction.extractors import (
    IndexVolExtractor,
...
class DataLoader:
    ...
    def load_all_terms(
        ...
        logger.info(f"Loading all terms for {index.portfolio} on {as_of_date}: {terms}")
        ...
        logger.info(f"Loaded {index.portfolio}: {valid_count}/{len(terms)} terms valid")
    ...
    def _load_vol_universe(
        ...
        terms: list[str],
        as_of_date: date,
    ) -> dict[str, VolSurfaceUniverse | None]:
        """
    ...
    def _build_data_package(
        ...
        vol_universes: dict[str, VolSurfaceUniverse | None],
        as_of_date: date,
    ) -> DataPackage | None:
        """
        ...
        try:
            ...
            if index_vol is None:
                logger.warning(f"Index vol not found for {index.portfolio} {term}")
                return None

            if not index_vol.is_valid():
                logger.warning(f"Index vol invalid for {index.portfolio} {term}")
                return None

            logger.debug(f"Index vol valid: {index_vol.summary()}")

        except Exception as e:
            logger.error(f"Error loading index vol {index.portfolio} {term}: {e}")
            return None
        ...
        try:
            ...
            logger.error(f"Error assembling DataPackage {index.portfolio} {term}: {e}")
            return None
...
class BatchDataLoader:
    ...
    def load_batch(
        ...
        for index in indices:
            ...
            for as_of_date in dates:
                logger.info(f"Loading {index.portfolio} on {as_of_date}: {terms}")
    ...
    ) -> dict[date, dict[str, DataPackage | None]]:
        """
