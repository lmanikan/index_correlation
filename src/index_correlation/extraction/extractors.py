    ...
    def extract(
        ...
        try:
            ...
            if df_filtered.empty:
                logger.warning(f"Index vol not found for {portfolio} {term}")
                return None
            ...
            logger.info(
                f"Loaded index vols {portfolio} {term}: {len(result.strikes)} strikes"
            )
            return result

        except Exception as e:
            logger.error(f"Failed to extract index vol {portfolio} {term}: {e}")
            return None
