...
class BackfillStats:
    ...
    def print_summary(self):
        ...
        logger.info(f"Backfill complete. Success: {self.successful}, Failed: {self.failed}, Skipped: {self.skipped}")
...
def backfill_correlations(
    ...
    with tqdm(total=stats.total_dates * stats.total_indices, desc="Backfilling") as pbar:
            ...
            for index in indices:
                try:
                    ...
                    if index.name in failed_indices and failed_indices[index.name] >= 3:
                        logger.warning(f"Skipping {index.name} on {current_date_py} - too many failures")
                        stats.record_skip()
                        pbar.update(1)
                        continue
                    
                    # Convert to datetime at 09:00 UTC (typically market open)
                    as_of = datetime.combine(current_date_py, datetime.min.time()).replace(hour=9)
                    ...
                    if package is None or len(package.ivol_surface) == 0:
                        logger.warning(f"Empty package for {index.name} {current_date_py}")
                        stats.record_skip()
                        pbar.update(1)
                        continue
                    ...
                    logger.error(f"✗ {index.name} {current_date_py}: {e}", exc_info=False)
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help='Database config name (postgres, bigquery, etc). Default: uses config file default'
    )
...
def main():
    ...
    try:
        if args.portfolio or args.symbol:
            indices = filter_indices(all_indices, portfolio=args.portfolio, symbol=args.symbol)
        else:
            indices = all_indices
