...
from index_correlation.extraction.market_data_extractor import (
    ExtractionError,
    MarketDataExtractor,
)
from index_correlation.extraction.validation import (
    ValidationError,
...
def compute_realtime(
    ...
    try:
        ...
        raise RealtimeComputeError(f"Initialization failed: {e}") from e
    ...
    try:
        ...
        raise RealtimeComputeError(f"Table creation failed: {e}") from e
        ...
        try:
            ...
            try:
                ...
                if validation_result["warnings"]:
                    logger.warning(
                        f"Validation warnings for {index_name}: "
            ...
            logger.error(
                f"Unexpected error processing {index_name}: {e}", exc_info=True
            )
            failure_count += 1
    ...
    # Summary
    logger.info(
        f"Real-time compute complete: {success_count} successes, "
        f"{failure_count} failures out of {len(INDICES_CONFIG)} indices"
    )

    if failures_list:
        logger.warning("Failed indices:")
    ...
    try:
        if args.analytics_engine_path:
            ...
            spec = importlib.util.spec_from_file_location(
                "analytics", args.analytics_engine_path
            )
        ...
        logger.info(
            "Note: Update --analytics_engine_path or adjust src/analytics.py import"
        )
        sys.exit(1)

    # Run compute
    try:
        success_count, failure_count, failures_list = compute_realtime(
            args.db_url, analytics_engine
        )
